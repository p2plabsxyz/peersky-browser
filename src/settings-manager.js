...
class SettingsManager {
  ...
  registerIpcHandlers() {
    ...
    ipcMain.handle("settings-set", async (event, key, value) => {
      try {
        ...
        logDebug(
          `Setting updated: ${key} changed from ${oldValue} to ${value}`
        );
  ...
  async loadSettings() {
    try {
      ...
      logDebug(
        `Settings loaded successfully: ${Object.keys(loaded).length} keys`
      );
      return this.settings;
    } catch (error) {
      if (error.code === "ENOENT") {
  ...
  validateSetting(key, value) {
    const validators = {
      ...
      customSearchTemplate: (v) =>
        typeof v === "string" && v.includes("%s") && v.length < 2048,
      theme: (v) =>
        [
          "transparent",
          "light",
  ...
  applySettingChange(key, value) {
    try {
      ...
      if (key === "theme") {
        ...
        logDebug(
          `Theme changed to ${value}, notified ${windows.length} windows`
        );
