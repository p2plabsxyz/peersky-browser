export class DownloadsPopup {
      ...
      setTimeout(() => {
        ...
        if (this.isVisible && this.popup) {
          ...
          if (this.activeDownloads.size === 0) {
            ...
            if (listContainer) { listContainer.innerHTML = '<div class="extensions-empty"><p>No active downloads</p></div>' }
          }
        }
      }, 1500)
    }
  ...
  updateSingleDownloadUI (dl) {
    ...
    if (!itemEl) {
      ...
      if (fileIcon) { this.loadSVG(fileIcon, 'peersky://static/assets/svg/download.svg') }
    } else {
  ...
  positionPopup () {
    ...
    if (left < 20) left = 20
    else if (left + popupRect.width > viewportWidth - 20) { left = viewportWidth - popupRect.width - 20 }
    if (top + popupRect.height > viewportHeight - 20) { top = buttonRect.top - popupRect.height - 8 }
    ...
    this.popup.addEventListener('click', (event) => {
      ...
      if (ctrlBtn) {
        ...
        if (ctrlBtn.classList.contains('toggle-pause')) {
          ...
          if (dl.isPaused || dl.state === 'interrupted') { this.ipc.send('download-resume', id) } else this.ipc.send('download-pause', id)
        }
  ...
  loadPopupSVGs (popup) {
    ...
    if (closeButton) { this.loadSVG(closeButton, 'peersky://static/assets/svg/close.svg') }
