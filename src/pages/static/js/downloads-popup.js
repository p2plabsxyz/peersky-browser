export class DownloadsPopup {
    ...
    if (data.state === "completed" || data.state === "cancelled") {
      setTimeout(() => {
        ...
        if (this.isVisible && this.popup) {
          ...
          if (this.activeDownloads.size === 0) this.hide();
        }
      }, 1500);
    }
  }
  ...
  updateSingleDownloadUI(dl) {
    ...
    if (!itemEl) {
      ...
      const escapedNameAttr = this.escapeHtmlAttribute(dl.filename);
      const escapedName = this.escapeHtml(dl.filename);

      itemEl.innerHTML = `
        <div class="extension-icon" role="img">
          <div class="svg-container file-svg"></div>
        </div>
        <div class="download-details" title="${escapedNameAttr}">
          <div class="extension-name">${escapedName}</div>
      ...
      const statusSpan = itemEl.querySelector(".status-text");
      const pauseBtn = itemEl.querySelector(".toggle-pause");
      ...
      if (dl.state === "completed" || dl.state === "cancelled") {
        const ctrlGrp = itemEl.querySelector(".ctrl-buttons");
        if (ctrlGrp) ctrlGrp.style.display = "none";
      }
    }
  }
